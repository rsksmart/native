...
public class NativeSecp256k1Test {
    ...
    public void testIsInfinityPointOfInfinitySig() throws NativeSecp256k1Exception {
        ...
        // data has already been signed with those {r,s}
        ...
        byte[] signature = NativeSecp256k1Util.concatenate(r,s,false); //TODO this should be removed and only use signCompact (check that behaviour)

        assertTrue(NativeSecp256k1.isInfinity(signature, data, 0)); // TODO should also add tests for recid 1, 2, 3, 4
    }
    ...
    @Test
    public void testIsInfinityRetrievedRSError() {
        String privateKey = String.join("", Collections.nCopies(64, "0"));
        testIsInfinityError(privateKey, NativeSecp256k1.RETRIEVED_R_S_ERROR);
    }

    private boolean isInfinity(String privateKeyString, byte[] data) throws NativeSecp256k1Exception {
    ...
    private void testIsInfinityError(String privateKeyString, String expectedErrorMessage) {
        byte[] data = BaseEncoding.base16().lowerCase().decode("f7cf90057f86838e5efd677f4741003ab90910e4e2736ff4d7999519d162d1ed".toLowerCase());
        try {
            isInfinity(privateKeyString, data);
